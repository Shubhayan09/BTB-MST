...
function loadOrdersFromFirebase() {
    try {
        const ordersRef = database.ref('orders');
        ordersRef.orderByChild('computerId').equalTo(currentUserId).on('value', (snapshot) => {
            orders = [];
...
function processAcceptedBid(bid) {
    ...
    // Create order records for both parties
    const buyerOrder = {
        userId: bid.action === "Buy" ? bid.senderId : bid.receiverId,
        team: bid.action === "Buy" ? bid.senderTeam : bid.receiverTeam,
        stock: bid.stock,
        price: bid.price,
        qty: bid.quantity,
        total: bid.total,
        status: bid.action === "Buy" ? "Buy" : "Sell",
        time: new Date().toLocaleString(),
        type: "Private",
        counterparty: bid.action === "Buy" ? bid.receiverTeam : bid.senderTeam,
        action: bid.action === "Buy" ? "Buy" : "Sell"
    };

    const sellerOrder = {
        userId: bid.action === "Sell" ? bid.senderId : bid.receiverId,
        team: bid.action === "Sell" ? bid.senderTeam : bid.receiverTeam,
        stock: bid.stock,
        price: bid.price,
        qty: bid.quantity,
        total: bid.total,
        status: bid.action === "Sell" ? "Sell" : "Buy",
        time: new Date().toLocaleString(),
        type: "Private",
        counterparty: bid.action === "Sell" ? bid.receiverTeam : bid.senderTeam,
        action: bid.action === "Sell" ? "Sell" : "Buy"
    };

    // Save both orders to Firebase
    saveOrderToFirebase(buyerOrder);
    saveOrderToFirebase(sellerOrder);

    // Update local orders array if this is our order
    if (bid.senderId === currentUserId || bid.receiverId === currentUserId) {
        orders.push(buyerOrder);
        orders.push(sellerOrder);
        renderOrdersTable();
    }

    // Update budgets and positions if this is our team
    if (bid.senderTeam === currentTeam || bid.receiverTeam === currentTeam) {
        if (bid.action === "Buy" && bid.senderTeam === currentTeam) {
            // Deduct money from buyer's budget
            remainingBudget -= bid.total;
            updateBudget();

            // Add to buyer's positions
            if (!positions[bid.stock]) {
                positions[bid.stock] = { qty: 0, buyLots: [], realizedPL: 0 };
            }
            positions[bid.stock].qty += bid.quantity;
            positions[bid.stock].buyLots.push({ qty: bid.quantity, price: bid.price });
        } 
        else if (bid.action === "Sell" && bid.receiverTeam === currentTeam) {
            // Add money to seller's budget
            remainingBudget += bid.total;
            updateBudget();

            // Remove from seller's positions
    ...
    try {
        ...
        bidsRef.orderByChild('receiverTeam').equalTo(currentTeam).on('value', (snapshot) => {
            ...
            snapshot.forEach((childSnapshot) => {
                ...
                if (bid.status === "Pending") {
                    ...
                    if (bid.senderTeam !== currentTeam && !bid.notificationShown) {
                        showNotification("New Bid Received", 
                            `${bid.senderTeam} has offered to ${bid.action.toLowerCase()} ${bid.stock} at ${formatCurrency(bid.price)}`);
                        // Mark as shown
                        bidsRef.child(childSnapshot.key).update({ notificationShown: true });
                    }
                }
            });
        ...
        bidsRef.orderByChild('senderId').equalTo(currentUserId).on('child_changed', (snapshot) => {
            ...
            if (bid.status === "Accepted" || bid.status === "Declined") {
                showNotification("Bid Update", 
                    `Your bid to ${bid.receiverTeam} for ${bid.stock} has been ${bid.status.toLowerCase()}`);
...
function renderOrdersTable() {
    ...
    orders.forEach(order => {
        html += `<tr>
            ...
            <td>${order.type}</td>
            <td>${order.time}</td>
            <td>${order.counterparty || 'Market'}</td>
        </tr>`;
    });
